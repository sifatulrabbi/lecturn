...
> `af_heart`) for the rest of the run — for a StepFun *or* an OpenRouter primary.
> This needs `OPENROUTER_API_KEY` set when the fallback fires; if it's missing
> you get the original error plus a note that the fallback was skipped. Pass
> `--fallback-model none` to disable it.
...
- Cache filenames embed a **fingerprint** of the **voice + response format +
  chunk text** — deliberately **not** the model or provider. So:
  ...
  - **Switching `--model` reuses the cache.** A book can legitimately span models
    and even providers (e.g. the StepFun→OpenRouter/Kokoro fallback on a quota
    outage), so the model isn't part of the key; each chunk is fingerprinted by
    the voice that actually narrated it. Use `--no-resume` to regenerate
    everything.
...
**Automatic cross-provider fallback:** if the primary provider rejects a request
for a quota/entitlement/unknown-model reason, `lecturn` switches — once, one-way,
for the rest of the run — to **OpenRouter + Kokoro-82M** (voice `af_heart`) and
continues there. This is the default for both providers. You'll see a `Note: fell
back from '…' to '…' (OpenRouter/Kokoro)` message, and the cost estimate reflects
the model actually used. The switch needs `OPENROUTER_API_KEY` at the moment it
fires; if it's unset you get the original error plus a note that the fallback was
skipped.
...
> Auth (`401`) and bad-voice errors never trigger a fallback (switching provider
> can't fix a bad key or a voice your account lacks).

Pass `--fallback-model none` to disable fallback entirely. When OpenRouter is
already the primary, the default fallback equals the primary model and is
therefore a no-op.
...
Kokoro is the only OpenRouter model, so an OpenRouter primary has nothing to fall
back to — the default `--fallback-model` (also Kokoro) is a no-op there. It is,
however, the fallback target for a **StepFun** primary.

> Kokoro caps input at **4096 tokens** per request. `lecturn` guards this
...
OpenRouter/Kokoro on a quota outage, set `OPENROUTER_API_KEY` too if you want that
safety net — it's only read when the fallback actually fires. `--dry-run`,
`list-models`, and `list-voices` need **no** key.
...
| `authentication failed` / HTTP 401 | Bad/missing key. Check `STEPFUN_API_KEY` (or `OPENROUTER_API_KEY` for `--provider openrouter`). (No fallback — switching provider can't fix a bad key.) |
| `… fallback to OpenRouter (Kokoro) was skipped because OPENROUTER_API_KEY is not set` | The primary provider failed and the automatic Kokoro fallback needs an OpenRouter key. Set `OPENROUTER_API_KEY`, or pass `--fallback-model none` to disable the fallback. |
