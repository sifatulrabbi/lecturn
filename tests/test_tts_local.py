"""Tests for the local (self-hosted Kokoro) TTS client.

Mirrors ``tests/test_tts_openrouter.py``: the network is never touched —
``_build_client`` is replaced with a fake and ``_request_audio`` is stubbed per
test. One test drives the *real* ``_request_audio`` through a fake OpenAI client
to prove the request always carries ``response_format="mp3"`` (Kokoro servers
default to PCM, like OpenRouter) plus the configured voice and model. The
``_explain`` guidance is local-specific and names the configured base URL.
"""
...
def _client(monkeypatch, **kw) -> LocalTTSClient:
...
def test_retries_then_succeeds(monkeypatch, tmp_path, mp3_bytes):
    ...
    def req(text, model):
        calls["n"] += 1
        if calls["n"] < 3:
            raise _RetryableError("connection", retry_after=0.001)
        return mp3_bytes
...
def test_auth_error_message_mentions_local_key(monkeypatch, tmp_path):
    c = _client(monkeypatch)

    def req(text, model):
        raise _FatalError("unauthorized", category="auth", fallback_eligible=False)

    c._request_audio = req
    with pytest.raises(TTSError) as exc:
        c.synthesize_text("hello", tmp_path / "a.mp3")
    # Local servers are usually unauthenticated; the hint names the opt-in var.
    assert "LOCAL_TTS_API_KEY" in str(exc.value)
    assert c.stats.failures == 1


def test_no_fallback_attempted_by_default(monkeypatch, tmp_path):
    # A bare LocalTTSClient (no wrapping FallbackTTSClient) never switches
    # providers on its own — a fallback-eligible error still raises after one
    # model attempt.
    c = _client(monkeypatch)
    ...
    def req(text, model):
        ...
        raise _FatalError("nope", category="quota", fallback_eligible=True)

    c._request_audio = req
    with pytest.raises(TTSError):
        c.synthesize_text("hello", tmp_path / "a.mp3")
    assert seen_models == ["kokoro"]   # exactly one model tried
...
def test_voice_error_message_points_at_list_voices(monkeypatch, tmp_path):
    ...
    with pytest.raises(TTSError) as exc:
        c.synthesize_text("hello", tmp_path / "a.mp3")
    assert "list-voices --provider local" in str(exc.value)


def test_model_error_message_names_base_url(monkeypatch, tmp_path):
    c = _client(monkeypatch)

    def req(text, model):
        raise _FatalError("unknown model", category="model", fallback_eligible=True)

    c._request_audio = req
    with pytest.raises(TTSError) as exc:
        c.synthesize_text("hello", tmp_path / "a.mp3")
    # The instance-method _explain names the configured base URL, the most
    # useful thing to check on a self-hosted server.
    assert "127.0.0.1:8880" in str(exc.value)
...
def test_request_sends_mp3_and_configured_voice_model(monkeypatch, tmp_path, mp3_bytes):
    ...
    Kokoro servers (like OpenRouter) default to raw PCM, which would break MP3
    cache validation and pydub stitching, so this asserts on the kwargs actually
    ...
    c = LocalTTSClient(config=_config())
